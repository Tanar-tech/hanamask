...
- [ ] **管理者のWindows環境の実インストールを更新し、更新前から存在するノート15件・タスク7件・リンク2件が変わらず開けること**（2026-08-08時点の実データ。退避先: `C:\Users\takuj\hanamask-backups\hanamask-2026-08-08-premigration.sqlite3`）
...
| やらないこと | 理由 |
|---|---|
| **本文の編集履歴** | ノートには履歴があるがタスクには無い。履歴用のテーブルと復元UIが要り、本文追加とは独立して設計できる |
| **本文を検索の対象にする** | 現在の検索はノート本文のみが対象。タスクを検索対象に含めるかは検索仕様そのものの変更になる |
| **タスクへの画像添付** | 画像はノート専用の仕組みとして作られている |
| **カンバンカードへの抜粋表示** | 上記のとおり意図的に出さない |

### 管理者に確認したいこと

1. **このリポジトリに初めて「DBマイグレーション」の仕組みを入れる。**現在は起動のたびにスキーマ定義を流すだけで、**既に存在するテーブルには一切手を加えない**作りになっている。このままでは、既存の利用者（管理者ご自身）のタスクに本文の列が追加されない。既存データを保ったまま列を足す仕組みを新設してよいか（`docs/GOVERNANCE.md` §6 のアーキテクチャ判断に該当）
