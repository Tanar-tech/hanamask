...
hanamask（ローカルのノート・タスク管理アプリ。MCPサーバーを内蔵）が接続されているとき、作業の記録・決定事項は hanamask に書く。頼まれるのを待たない。

- **書く**: 作業の区切り、方針や仕様の決定、調べて分かったこと、次に持ち越すこと。\`create_note\` / \`create_task\` を使い、決定はその理由ごと本文にMarkdownで残す。タグを付けて案件を判別できるようにする（\`list_tags\` で既存のタグを再利用する）
- **育てる（新規作成より更新を優先する）**: 同じ話題のノート・タスクが既にあれば、新しく作らず \`update_note\` / \`update_task\` で追記・修正する。続きの作業は既存ノートに経過を書き足し、タスクは着手時に \`in_progress\`、終わったら \`done\` へステータスを進める。1つの話題が複数ノートに散ると、次に検索したとき全部を読まないと経緯が分からなくなる
...
- **読む**: 作業を始める前に \`search_notes\` で過去の経緯を引く。同じ調査を二度やる・決定済みの方針を蒸し返すのを防ぐ。引いたノートの内容が古くなっていたら、その場で更新する
