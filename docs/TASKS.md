...
| タスク | 残っている内容 | 必須依存 |
|---|---|---|
| **T31** | **WSLでの開発体験を整える（優先度: 高）** | T13, T12 |
...
- ステータス: 未着手（**スコープの確認が必要**。下記「停止条件」参照）
- 優先度: **高**（管理者指示、2026-08-06）
- 依存: 必須: T13（パッケージング）, T12（チャットのAPIキー保管）／推奨: なし
- 目的: このリポジトリはWSL上で開発されているが、**WSLだからこそ手作業や回避策が必要な箇所が複数あり、その都度コストを払っている**。以下はいずれも本セッションで実際に発生したもの。
  1. **インストーラーのビルドが手作業**。`npm run package:win` はWSLのLinux側では完走しない（`node-gyp`がクロスコンパイル非対応）。Windows側のNodeを呼ぶ必要があるが、UNCパス（`\\wsl$`）上では`npm`が正しく動かないため、**ソースをWindowsネイティブパスへ手でコピーしてから実行**している（`docs/PACKAGING.md` §2）。スクリプト化されていない。
  2. **`safeStorage`がWSLでは使えない**。T12でAPIキーは`safeStorage`で暗号化保存する設計にし、暗号化が使えない環境では保存を断る実装にした。その結果、**WSL上の開発環境ではAPIキーを保存できず、AIチャットを動かせない**。実機Windowsでしか確認できない機能が生まれている。
  3. **E2Eのスクリーンショットで日本語が豆腐になる**。Linux側に日本語フォントが無く、`FONTCONFIG_FILE`でWindows側のフォントを参照する回避策が必要（`.claude/skills/e2e-runner/SKILL.md`に記載済みだが、リポジトリのスクリプトには入っていない）。
- 変更範囲: `scripts/`（Windows側ビルドの自動化）, `package.json`（スクリプト追加）, `docs/PACKAGING.md`, `.claude/skills/e2e-runner/SKILL.md`, および必要に応じて`src/main/settings/chat-settings.ts`（開発時の扱い）。**アプリの本番挙動を変える変更は慎重に扱う**（下記禁止事項）。
- 禁止事項: **本番で`safeStorage`を迂回する経路を作らないこと。** 開発時の利便のために平文保存へフォールバックすると、利用者のAPIキーが平文で置かれる事故に直結する。開発用の緩和を入れる場合は、本番ビルドに絶対に混入しないことをテストで固定する（T27のCSPで同じ問題を扱った前例がある）。パッケージングの自動化にあたり、`docs/PACKAGING.md`に記録済みの制約（クロスコンパイル不可、UNCパス不可、VS Build Toolsの`isComplete`確認）を回避しようとしないこと。
- テスト: ビルド自動化スクリプトは、**実際にWSLから実行して`.exe`が生成されるところまで確認する**（手順を書いただけで終わらせない）。開発時の緩和を入れる場合は、本番ポリシーに混入しないことの単体テストを必須とする。日本語フォントの対応は、スクリーンショットに豆腐が出ないことを実機で確認する。
- 停止条件: **「WSL対応」の意図するスコープを管理者に確認してから着手する。** 上記1〜3は本セッションで観測した具体的な痛点だが、他に意図している範囲（例: WSLgでのGUI起動、Windows側とLinux側でのDB共有、開発環境の完全なWindows移行など）があれば先に確認する。`safeStorage`まわりで本番挙動を変える必要が生じた場合も、秘密情報の取り扱いに直結するため必ず確認する。
