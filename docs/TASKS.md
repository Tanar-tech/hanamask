...
- 管理者確認済みの方針（2026-08-05）:
  ...
  2. **UIライブラリ**: UIコンポーネントライブラリの**追加を承認**（`docs/GOVERNANCE.md` §6の依存関係追加について事前承認済み）。具体的な採用ライブラリはモックアップ提示時に併せて提案し、確定させる。
  3. **進め方**: **モックアップ承認 → 画面単位で段階的に置換**。デザイン方針と主要画面のイメージを提示して承認を得てから実装に入り、画面ごとにPRを分ける。
- 停止条件:
  - モックアップの承認を得るまで実装（`src/renderer/`の書き換え）に着手しない。
  - 採用するUIライブラリはモックアップと同時に提案し、管理者の確定を得てから`package.json`に追加する（「追加してよい」の承認は得ているが、どれを入れるかは未確定）。
