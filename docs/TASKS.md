...
| 2 | T06（リンク機能） | **進行中**（バックエンドはマージ済み、リンク表示・作成UIはレビュー中で未マージ） | T03, T05 |
...
主要機能タスクはT06のUI分とT12を除きマージ済み。残っているのは以下のみで、いずれも相互に独立して並列着手できる。

| タスク | 残っている内容 | 必須依存 |
|---|---|---|
| T06 | リンク表示・作成UIのレビュー完了とマージ（バックエンドは完了済み） | T03, T05 |
| T12 | AIチャットパネル（BYO Agent）。未着手・最大の残スコープ | T03, T05 |
| T16 | ソフトデリート済みノートの一覧・復元UI（T03から分離） | T03 |
| T17 | MCP経由のノート本体更新を、開いている詳細画面へ反映する | T15 |
| T18 | T04履歴UIの復元レース修正（reviewerのMajor指摘） | T04 |
| T19 | Mermaid描画失敗時のエラーSVGのDOM残留（T07の既知の見た目の問題） | T07 |
...
- ステータス: 進行中（バックエンドはマージ済み＝PR #13。リンク表示・作成UIは実装済みだが**現在レビュー中で未マージ**）
...
- 状況（UI分）: T15でノート/タスク詳細画面が完成したため組み込み先が確保され、リンク表示・作成UIの実装に着手済み。実装は完了しているがレビューが継続中で`main`には未マージ。マージ完了をもって本タスクを完了とする。
