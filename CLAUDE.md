...
- **ループ・長時間セッションの作業記録は、リポジトリ内の作業ログと hanamask アプリの両方に残す。**置き場の分担は下記「作業記録の置き場」に従う。
- AWSサーバーレス構成・PRプレビュー環境・クラウドデプロイパイプラインは廃止した（ローカル完結アプリのため不要）。配布はインストーラーの再配布による（docs/GOVERNANCE.md §3.1）。

## 作業記録の置き場

このリポジトリには `docs/TASKS.md`・`STATE.md`・`loop-run-log.md` という作業ログが揃っているため、グローバル規約の「リポジトリ内の作業ログがある場合はそちらが正」だけを読むと、**hanamask アプリには一切書かれない**。実際に、MCPが接続可能な状態で数日間まったく使われなかった（README §3）。本リポジトリでは以下のとおり分担する。

| 置き場 | 何を書くか |
| --- | --- |
| `docs/TASKS.md` | タスクの正。起票・ステータス・完了条件。機械判定と管理者レビューの対象 |
| `STATE.md` / `loop-run-log.md` | ループの実行状態と Watch List。次の実行が最初に読む |
| hanamask（MCP） | 実行ごとの経過と判断の理由。上記3ファイルに残らない「なぜその順で手を付けたか」「何を見送ったか」 |

hanamask 側の書き方はグローバル規約「作業の記録は hanamask に書く」に従う。本リポジトリでの具体は次のとおり。

- 案件（タスク番号）ごとに**ノートを1つ**作り、実行のたびに新しいページを足さず `update_page` で既存ページを育てる。区切りでは `update_notebook` でノートの概要を現在地に合わせて書き直す
- `docs/TASKS.md` に起票したタスクは hanamask 側にも `create_task` で立て、着手時に `in_progress`、完了時に `done` へ進める。**両者が食い違ったら `docs/TASKS.md` が正**
- 実行の冒頭で `search_notes` / `semantic_search_notes` を引き、前回の経過を読んでから着手する
- **アプリが起動していなければ書かない。**MCPツールが見えないときは無理に接続を試みず、リポジトリ内の作業ログだけを更新し、書けなかったことを報告する
