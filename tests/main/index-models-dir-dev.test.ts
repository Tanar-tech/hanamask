...
// E2E は非パッケージ実行なので、環境変数での上書きが効き続けることを固定する。
describe("開発・E2E のモデル置き場", () => {
  beforeAll(async () => {
  ...
  it("HANAMASK_MODELS_DIR で置き場を上書きできる", () => {
    expect(loadEmbeddingProvider).toHaveBeenCalledWith(OVERRIDE_DIR);
    expect(loadEmbeddingProvider).not.toHaveBeenCalledWith(join(RESOURCES_PATH, "models"));
  });
});
