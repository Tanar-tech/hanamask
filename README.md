...
### 2. 利用しているAIエージェントにて下記のプロンプトを実行する

```
hanamask というローカルアプリの MCP サーバーにつないでください。
エンドポイントは http://127.0.0.1:39217/mcp、トランスポートは Streamable HTTP です。
つないだら、使えるツールを一覧して教えてください。
```

エージェントが接続の設定を済ませ、`create_note` や `create_task` などが見えていれば準備完了。
...
> **うまくつながらないときは。**hanamask が起動しているかを確認する。MCPサーバーはアプリの中で動いているため、**アプリを閉じるとエージェントからも見えなくなる。**手で設定する場合の手順は[MCPサーバーへの接続](#mcpサーバーへの接続)にある。
